...
ignore_list = ['accurateAlphaTest', 'beforeDraw', 'bilinearUpscale', 'cpuSpriteRenderLevel', 'drawBuffering',  'eeCycleRate', 'getSkipCount', 'gpuTargetCLUT', 'limit24BitDepth', 'maximumBlendingLevel', 'minimumBlendingLevel', 'name-sort', 'nativePaletteDraw', 'nativeScaling', 'partialTargetInvalidation', 'PCRTCOffsets', 'PCRTCOverscan', 'readTCOnClose', 'recommendedAccurateAlphaTest', 'recommendedBlendingLevel', 'recommendedHWAA1']
...
def process_dict(my_dict, new_dict):
    ...
    for key, value in new_dict.items():
        ...
        if key in speedfix_dict and key in my_dict:
            ...
            if 'Ace Combat 04' in my_dict[key]['name']: my_dict[key]['gsHWFixes']['cpuCLUTRender'] = 1
...
if os.path.isfile('GameIndex[temp2].yaml'): os.remove('GameIndex[temp2].yaml')
with open('GameIndex[converted].yaml', encoding='utf8') as base, open('old/GameIndex[3668].yaml', encoding='utf8') as og, open('GameIndex[override].yaml', encoding='utf8') as diff, open('GameIndex[merged].yaml', 'w', encoding='utf8') as merged:
